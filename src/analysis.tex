...
Requiring that the transverse momentum of these resonances is very large (highly boosted) offers a path forward to both triggering on the interesting signal events and also reducing the multijet background which exponentially falls of with increasing momentum.
To achieve this highly boosted state, the resonance can recoil off a high energy jet that it is produced through initial state radiation or other similar process, as shown in the leading order at LHC Feynman diagrams in \Cref{fig:signal_LO_diagrams} for signal models of a leptophobic $\Zprime$ and the Higgs boson.
...
By applying $b$-tagging to the signal \largeR{} jet and requiring the presence of two $b$-tagged jets inside the performance of the analysis can be improved.
...
In some models the effect of the anomalous couplings is very significant and would affect the production cross section of high-$p_{T}$ Higgs through gluon-gluon fusion up to $50\%$~\cite{Grojean:2013nya,Dawson:2015gka}.
...
This chapter will cover the signal models and datasets in \Cref{sec:data_and_simulation}, and the $p_{T}$ requirements to have a fully efficient \largeR{} jet trigger across the 2015, 2016, and 2017 datasets is discussed in \Cref{sec:analysis_trigger}.
...
The analysis was done using $80.5~\ifb$ of data from ATLAS spanning datasets collected in 2015, 2016, and 2017.
...
The decay of the resulting $b$-hadrons if performed with EvtGen~\cite{Lange:2001uf}.
Similarly, with the exception of HJ+MiNLO, the VBF Higgs events are generated in the same manner.
Likewise, the Higgsstrahlung Higgs events are generated using \textsc{Pythia} 8.212 with the AZNLO tune and the CTEQ6L1 parton distribution function, with the decay of $b$-hadrons done with EvtGen.
...
In brief, Wilk's theorem~\cite{Wilks:1938dza} states that for two functions in a nested family of functions, $f\left(x\,;\,\vec{\theta}_{a}\right)$ and $f\left(x\,;\,\vec{\theta}_{b}\right)$, with the number of parameters $a < b$, in the limit of large sample size the likelihood ratio test statistic,
\begin{equation}
...
The one-sided (one-tailed) \pvalue{} --- given Equation~\ref{eq:wilks_theorem} as the null hypothesis --- is then the value of the complementary cumulative distribution function of the $\chi^2_{d}$ distribution evaluated at the observed $t_{\vec{\theta}}$,
\begin{equation}
...
To check the bias for a signal model of a Higgs boson the signal injection tests with smoothed Sherpa Monte Carlo templates were run again, but with a smoothed Higgs templates for a signal model instead of a $\Zprime$.
