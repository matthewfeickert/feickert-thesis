...
As before in \Cref{section:symmetry_breaking} with the case of the toy model of the complex scalar field, once a ground state has been arbitrarily chosen this spontaneously breaks the $\mathrm{SU}(2)_{L} \otimes \mathrm{U}(1)_{Y}$ symmetry to the subgroup $\mathrm{U}(1)_{\textrm{QED}}$.
...
\[
 \Lagrangian_{\textrm{vector}} = g_{q} \sum_{q} Z'_{\mu} \bar{q} \gamma^{\mu} q + g_{\chi} Z'_{\mu} \bar{\chi} \gamma^{\mu} \chi
\]
and for an axial-vector model
\[
 \Lagrangian_{\textrm{axial-vector}} = g_{q} \sum_{q} Z'_{\mu} \bar{q} \gamma^{\mu}\gamma^{5} q + g_{\chi} Z'_{\mu} \bar{\chi} \gamma^{\mu}\gamma^{5} \chi
\]
...
\TODO{Explain why for the searches done only axial-vector models are considered. It is not clear to me why.}
