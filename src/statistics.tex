...
As $\hat{\hat{\vec{\theta}}}$ is a function of the given $\mu$ then it is seen that the likelihood function's dependence on the nuisance parameters $\vec{\theta}$ can be ``profiled'' as a function of $\mu$ and removed.
...
Given that the negative log likelihood is usually calculated for numerical reasons, the test statistic that is generally used from the profile likelihood ratio is
\begin{equation}
...
Alternatively, if the test statistic used is the profile likelihood ratio,%
\footnote{$\hat{\theta}$ is the maximum likelihood estimator of $\theta$, and $\hat{\hat{\theta}}$ is the conditional maximum likelihood estimate for $\theta$ for a given $t$.}
\[
 \lambda\left(t\right) = \frac{L\left(t, \hat{\hat{\theta}}\,\right)}{L\left(\hat{t},\hat{\theta}\right)}\,,
\]
profiling determines the allowed range $\left[t_{1}, t_{2}\right]$.
